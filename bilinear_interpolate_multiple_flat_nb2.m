function [colors] = bilinear_interpolate_multiple_flat_nb2(flat_texture, dim, channels, points)
  % same as bilinear_interpolate_multiple_flat_nb, but assumes
  % transposed points which might be better on the cache since matlab
  % matrices are stored column-wise
  %
  % however, it does not seem faster (maybe because of the extra
  % transpositions I had to introduce)
