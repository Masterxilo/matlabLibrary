function displaySIFTPatches(positions, scales, orients)
...
%
% It only adds the patches to the current axes, assuming you have already
% displayed the image.

assert(isvector(orients));
assert(all(orients >= -2*pi));
assert(all(orients <= 2*pi));
